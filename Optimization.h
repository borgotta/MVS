...
namespace MVS {
	...
	public:
		...
		void addImages(Patch& patch) const;
		void removeImagesEdge(Patch& patch) const;

		float getUnit(const int index, const Vec4f& coord) const;

		void computeUnits(const Patch& patch,
	...
	protected:
		...
		void sortImages(Patch& patch) const;
	...
	public:
		...
		// Optimization
		//-----------------------------------------------------------------
		int preProcess(Patch& patch, const int id, const int seed);
		void refinePatch(Patch& patch, const int id, const int time);

		void refinePatchBFGS(Patch& patch, const int id, const int time);
		void refinePatchBFGS(Patch& patch, const int id, const int time,
			const int ncc);
		bool refinePatchBFGS2(Patch& patch, const int id, const int time,
			const int ncc); // LM version
		void refineDepthBFGS(Patch& patch, const int id, const int time,
			const int ncc);

		int postProcess(Patch& patch, const int id, const int seed);

		void setRefImage(Patch& patch, const int id);
	...
	protected:
		...
		void setRefConstraintImages(Patch& patch, const float nccThreshold,
			const int id);

		void setINCCs(const Patch& patch,
	...
	public:
		void setWeightsT(const Patch& patch, const int id);
